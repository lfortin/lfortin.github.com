...
`,i=Backbone.View.extend({id:`home`,tagName:`div`,events:{"click a.button-counter":`onCounterClick`},onCounterClick(e){e.stopPropagation(),e.preventDefault();let t=$(e.target).closest(`.card`).attr(`data-model-id`),n=this.collection.get(t);n.set(`counter`,n.get(`counter`)+1)},initialize(){this.collection=new t,this.listenTo(this.collection,`update reset change`,this.render)},template:Handlebars.compile(r),render(){let e=this.template({usecases:this.collection.toJSON()});return this.$el.html(e),this}}),a=n`
  <div class="container mt-4">
    <h2>About</h2>
    <p>This is a Vite + Backbone starter template using Bootstrap, Handlebars, and more.</p>
    <p>Released under the MIT-0 License.</p>
  </div>
`,o=Backbone.View.extend({id:`about`,tagName:`div`,render(){return this.$el.html(a),this}}),s=n`
  <div class="container mt-4">
    <h2>Help</h2>
    <p>
...
`,c=Backbone.View.extend({id:`help`,tagName:`div`,render(){return this.$el.html(s),this}}),l=n`
  <div class="container mt-4">
    <h2 class="text-danger"><i class="bi bi-exclamation-triangle"></i> 404 Not Found</h2>
    <p>The page you requested could not be found.</p>
  </div>
`,u=Backbone.View.extend({id:`not-found`,tagName:`div`,render(){return this.$el.html(l),this}}),d=Backbone.Model.extend({idAttribute:`slug`,defaults:{slug:``,title:``,date:``,excerpt:``,content:``,tags:[]}}),f=Backbone.Collection.extend({model:d}),p=5,m=n`
...
`,h=Backbone.View.extend({id:`blog`,tagName:`div`,initialize(){this.collection=new f,this.currentPage=1,this.listenTo(this.collection,`update reset`,this.render)},setPage(e){let t=this._getTotalPages();this.currentPage=Math.min(Math.max(e,1),t||1),this.render()},_getTotalPages(){if(!this.collection)return 1;let e=this.collection.length;return e>0?Math.ceil(e/p):1},template:Handlebars.compile(m),render(){let e=this._getTotalPages(),t=this.currentPage||1,n=(t-1)*p,r=n+p,i=(this.collection?this.collection.slice(n,r):[]).map(e=>e.toJSON()),a=[];for(let n=1;n<=e;n+=1)a.push({number:n,isCurrent:n===t});let o=this.template({posts:i,hasPagination:e>1,pages:a,isFirstPage:t===1,isLastPage:t===e,prevPage:t-1,nextPage:t+1});return this.$el.html(o),this}}),g=n`
...
`,_=Backbone.View.extend({id:`blog-post`,tagName:`div`,initialize(){this.collection=new f,this.currentPost=null},showBySlug(e){if(!this.collection)return!1;let t=this.collection.get(e);return t?(this.currentPost=t,this.render(),!0):(this.currentPost=null,this.render(),!1)},template:Handlebars.compile(g),render(){let e=this.template({post:this.currentPost?this.currentPost.toJSON():null});return this.$el.html(e),this}}),v=[{id:`cc895078-2ab0-45e8-b346-2a4d025647c0`,title:`Minimal View Convention`,description:`Backbone lets you structure your app into small reusable views.`,link:`https://backbonejs.org/#View`},{id:`eae3cb6c-d6fb-4007-8afd-7d2e32c5b759`,title:`Logic + Layout Separation`,description:`Handlebars templating keeps your HTML clean and logic-free.`,link:`https://handlebarsjs.com/`},{id:`5916615a-0fca-4854-bfc4-c1ffdbea9337`,title:`Quick Styling`,description:`Bootstrap helps rapidly layout components with minimal CSS.`,link:`https://getbootstrap.com/docs/5.3/components/card/`},{id:`177aa50c-a466-4c56-987c-c3c794e4fb8d`,title:`Data Binding & Events`,description:`Backbone’s models and collections support change events and syncing with RESTful APIs.`,link:`https://backbonejs.org/#Model`},{id:`fabe99de-cc77-4d37-9e97-decc3e73fa15`,title:`Client-Side Routing`,description:`Use Backbone.Router to manage application state and navigation.`,link:`https://backbonejs.org/#Router`},{id:`03b7a643-52db-4daa-a5b0-f276782cfcca`,title:`Unit Testing`,description:`Vitest is a fast, powerful, and easy-to-use JavaScript test runner.`,link:`https://vitest.dev/`},{id:`77ed2189-0949-45cd-9746-3ad82b1df137`,title:`Fast Dev with Vite`,description:`Vite enables blazing fast HMR and optimized production builds.`,link:`https://vitejs.dev/`},{id:`dd6eaa1e-f0dc-4f9c-87cc-3f9423a901e6`,title:`Oxlint for Code Quality`,description:`Oxlint statically analyzes your code to find problems and enforce style.`,link:`https://oxc.rs/docs/guide/usage/linter.html`},{id:`b975384c-6294-4d51-a87d-b5092078ad2f`,title:`Single Page App Foundation`,description:`Backbone is ideal for small to medium SPAs with clear structure.`,hasCounter:!0}],y=[{slug:`2026-02-28-getting-started-with-vite-backbone`,date:`2026-02-28`,title:`Getting Started with Vite + Backbone`,excerpt:`Learn how this starter template wires together Vite, Backbone, Handlebars, jQuery, and Bootstrap to build small SPAs.`,content:`
    ...
    `,tags:[`testing`,`vitest`]}],b=new i,x=new o,S=new c,C=new h,w=new _,T=new u;new(Backbone.Router.extend({routes:{"":`home`,about:`about`,help:`help`,blog:`blogIndex`,"blog/page/:page":`blogIndex`,"blog/:slug":`blogPost`,"*path":`notFound`},initialize(){$(`#app`).empty(),$(`#app`).append(b.$el),$(`#app`).append(x.$el),$(`#app`).append(S.$el),$(`#app`).append(C.$el),$(`#app`).append(w.$el),$(`#app`).append(T.$el)},_showView(e){e?.$el&&($(`#app > div`).not(e.el).hide(),e.$el.show())},home(){b.collection.reset(v),this._showView(b)},about(){this._showView(x.render())},help(){this._showView(S.render())},blogIndex(e=1){C.collection.reset(y),C.setPage(e),this._showView(C)},blogPost(e){if(w.collection.reset(y),!w.showBySlug(e)){this.notFound();return}this._showView(w)},notFound(){this._showView(T.render())}})),Backbone.history.start(),$(function(){});