...
`,h=Backbone.View.extend({id:`blog`,tagName:`div`,initialize(){this.collection=new f,this.currentPage=1,this.listenTo(this.collection,`update reset change`,this.render)},setPage(e){let t=this._getTotalPages();this.currentPage=Math.min(Math.max(e,1),t||1),this.render()},_getTotalPages(){if(!this.collection)return 1;let e=this.collection.length;return e>0?Math.ceil(e/p):1},template:Handlebars.compile(m),render(){let e=this._getTotalPages(),t=this.currentPage||1,n=(t-1)*p,r=n+p,i=(this.collection?this.collection.slice(n,r):[]).map(e=>e.toJSON()),a=[];for(let n=1;n<=e;n+=1)a.push({number:n,isCurrent:n===t});let o=this.template({posts:i,hasPagination:e>1,pages:a,isFirstPage:t===1,isLastPage:t===e,prevPage:t-1,nextPage:t+1});return this.$el.html(o),this}}),g=n`
...
`,v=Backbone.View.extend({id:`blog-post`,tagName:`div`,initialize(){this.model=new d,this.listenTo(this.model,`change`,this.render)},template:Handlebars.compile(g),render(){let e=this.template({post:this.model?this.model.toJSON():null});return this.$el.html(e),this}}),y=[{id:`cc895078-2ab0-45e8-b346-2a4d025647c0`,title:`Minimal View Convention`,description:`Backbone lets you structure your app into small reusable views.`,link:`https://backbonejs.org/#View`},{id:`eae3cb6c-d6fb-4007-8afd-7d2e32c5b759`,title:`Logic + Layout Separation`,description:`Handlebars templating keeps your HTML clean and logic-free.`,link:`https://handlebarsjs.com/`},{id:`5916615a-0fca-4854-bfc4-c1ffdbea9337`,title:`Quick Styling`,description:`Bootstrap helps rapidly layout components with minimal CSS.`,link:`https://getbootstrap.com/docs/5.3/components/card/`},{id:`177aa50c-a466-4c56-987c-c3c794e4fb8d`,title:`Data Binding & Events`,description:`Backbone’s models and collections support change events and syncing with RESTful APIs.`,link:`https://backbonejs.org/#Model`},{id:`fabe99de-cc77-4d37-9e97-decc3e73fa15`,title:`Client-Side Routing`,description:`Use Backbone.Router to manage application state and navigation.`,link:`https://backbonejs.org/#Router`},{id:`03b7a643-52db-4daa-a5b0-f276782cfcca`,title:`Unit Testing`,description:`Vitest is a fast, powerful, and easy-to-use JavaScript test runner.`,link:`https://vitest.dev/`},{id:`77ed2189-0949-45cd-9746-3ad82b1df137`,title:`Fast Dev with Vite`,description:`Vite enables blazing fast HMR and optimized production builds.`,link:`https://vitejs.dev/`},{id:`dd6eaa1e-f0dc-4f9c-87cc-3f9423a901e6`,title:`Oxlint for Code Quality`,description:`Oxlint statically analyzes your code to find problems and enforce style.`,link:`https://oxc.rs/docs/guide/usage/linter.html`},{id:`b975384c-6294-4d51-a87d-b5092078ad2f`,title:`Single Page App Foundation`,description:`Backbone is ideal for small to medium SPAs with clear structure.`,hasCounter:!0}],b=[{slug:`2026-02-28-getting-started-with-vite-backbone`,date:`2026-02-28`,title:`Getting Started with Vite + Backbone`,excerpt:`Learn how this starter template wires together Vite, Backbone, Handlebars, jQuery, and Bootstrap to build small SPAs.`,content:`
    ...
    `,tags:[`testing`,`vitest`]}],x=new i,S=new o,C=new c,w=new h,T=new v,E=new u;new(Backbone.Router.extend({routes:{"":`home`,about:`about`,help:`help`,blog:`blogIndex`,"blog/page/:page":`blogIndex`,"blog/:slug":`blogPost`,"*path":`notFound`},initialize(){$(`#app`).empty(),$(`#app`).append(x.$el),$(`#app`).append(S.$el),$(`#app`).append(C.$el),$(`#app`).append(w.$el),$(`#app`).append(T.$el),$(`#app`).append(E.$el)},_showView(e){e?.$el&&($(`#app > div`).not(e.el).hide(),e.$el.show())},home(){x.collection.reset(y),this._showView(x)},about(){this._showView(S.render())},help(){this._showView(C.render())},blogIndex(e=1){w.collection.reset(b),w.setPage(e),this._showView(w)},blogPost(e){let t=_.findWhere(b,{slug:e});if(!t){this.notFound();return}T.model.set(t),this._showView(T)},notFound(){this._showView(E.render())}})),Backbone.history.start(),$(function(){});