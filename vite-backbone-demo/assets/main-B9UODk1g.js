...
`,y=Backbone.View.extend({id:"home",tagName:"div",events:{"click a.button-counter":"onCounterClick"},onCounterClick(e){e.stopPropagation(),e.preventDefault();const s=$(e.target).closest(".card").attr("data-model-id"),i=this.collection.get(s);i.set("counter",i.get("counter")+1)},initialize(){this.collection=new v,this.listenTo(this.collection,"update reset change",this.render)},template:Handlebars.compile(k),render(){const e=this.template({usecases:this.collection.toJSON()});return this.$el.html(e),this}}),x=`
  <div class="container mt-4">
    <h2>About</h2>
...
`,I=Backbone.View.extend({id:"blog",tagName:"div",events:{"click .pagination .page-link":"onPageClick"},initialize(){this.collection=new m,this.currentPage=1,this.listenTo(this.collection,"update reset",this.render)},setPage(e){const s=this._getTotalPages(),i=Math.min(Math.max(e,1),s||1);this.currentPage=i,this.render()},_getTotalPages(){if(!this.collection)return 1;const e=this.collection.length;return e>0?Math.ceil(e/d):1},onPageClick(e){e.preventDefault();const i=$(e.currentTarget).data("page"),o=this._getTotalPages();let t=this.currentPage;if(i==="prev")t=this.currentPage-1;else if(i==="next")t=this.currentPage+1;else{const a=parseInt(i,10);Number.isNaN(a)||(t=a)}t<1||t>o||(this.currentPage=t,this.render())},template:Handlebars.compile(_),render(){const e=this._getTotalPages(),s=this.currentPage||1,i=(s-1)*d,o=i+d,a=(this.collection?this.collection.slice(i,o):[]).map(l=>l.toJSON()),n=[];for(let l=1;l<=e;l+=1)n.push({number:l,isCurrent:l===s});const f=this.template({posts:a,hasPagination:e>1,pages:n,isFirstPage:s===1,isLastPage:s===e});return this.$el.html(f),this}}),N=`
...
`,H=Backbone.View.extend({id:"blog-post",tagName:"div",initialize(){this.collection=new m,this.currentPost=null},showBySlug(e){if(!this.collection)return!1;const s=this.collection.get(e);return s?(this.currentPost=s,this.render(),!0):(this.currentPost=null,this.render(),!1)},template:Handlebars.compile(N),render(){const e=this.template({post:this.currentPost?this.currentPost.toJSON():null});return this.$el.html(e),this}}),L=[{id:"cc895078-2ab0-45e8-b346-2a4d025647c0",title:"Minimal View Convention",description:"Backbone lets you structure your app into small reusable views.",link:"https://backbonejs.org/#View"},{id:"eae3cb6c-d6fb-4007-8afd-7d2e32c5b759",title:"Logic + Layout Separation",description:"Handlebars templating keeps your HTML clean and logic-free.",link:"https://handlebarsjs.com/"},{id:"5916615a-0fca-4854-bfc4-c1ffdbea9337",title:"Quick Styling",description:"Bootstrap helps rapidly layout components with minimal CSS.",link:"https://getbootstrap.com/docs/5.3/components/card/"},{id:"177aa50c-a466-4c56-987c-c3c794e4fb8d",title:"Data Binding & Events",description:"Backbone’s models and collections support change events and syncing with RESTful APIs.",link:"https://backbonejs.org/#Model"},{id:"fabe99de-cc77-4d37-9e97-decc3e73fa15",title:"Client-Side Routing",description:"Use Backbone.Router to manage application state and navigation.",link:"https://backbonejs.org/#Router"},{id:"03b7a643-52db-4daa-a5b0-f276782cfcca",title:"Unit Testing",description:"Vitest is a fast, powerful, and easy-to-use JavaScript test runner.",link:"https://vitest.dev/"},{id:"77ed2189-0949-45cd-9746-3ad82b1df137",title:"Fast Dev with Vite",description:"Vite enables blazing fast HMR and optimized production builds.",link:"https://vitejs.dev/"},{id:"dd6eaa1e-f0dc-4f9c-87cc-3f9423a901e6",title:"ESLint for Code Quality",description:"ESLint statically analyzes your code to find problems and enforce style.",link:"https://eslint.org/"},{id:"b975384c-6294-4d51-a87d-b5092078ad2f",title:"Single Page App Foundation",description:"Backbone is ideal for small to medium SPAs with clear structure.",hasCounter:!0}],h=[{slug:"2026-02-28-getting-started-with-vite-backbone",date:"2026-02-28",title:"Getting Started with Vite + Backbone",excerpt:"Learn how this starter template wires together Vite, Backbone, Handlebars, jQuery, and Bootstrap to build small SPAs.",content:`
    ...
    `,tags:["testing","vitest"]}],p=new y,u=new B,g=new V,r=new I,c=new H,b=new C,R=Backbone.Router.extend({routes:{"":"home",about:"about",help:"help",blog:"blogIndex","blog/:slug":"blogPost","*path":"notFound"},initialize(){$("#app").empty(),$("#app").append(p.$el),$("#app").append(u.$el),$("#app").append(g.$el),$("#app").append(r.$el),$("#app").append(c.$el),$("#app").append(b.$el)},_showView(e){e?.$el&&($("#app > div").not(e.el).hide(),e.$el.show())},home(){p.collection.reset(L),this._showView(p)},about(){this._showView(u.render())},help(){this._showView(g.render())},blogIndex(){r.collection.reset(h),r.setPage(1),this._showView(r)},blogPost(e){if(c.collection.reset(h),!c.showBySlug(e)){this.notFound();return}this._showView(c)},notFound(){this._showView(b.render())}});new R;Backbone.history.start();$(function(){});
